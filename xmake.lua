            ...
            for _, pkg in ipairs(target:orderpkgs()) do
                ...
                if pkg:enabled() and pkg:get("libfiles") then
                    for _, dll_path in ipairs(table.wrap(pkg:get("libfiles"))) do
                        if
                            (target:is_plat("windows", "mingw") and dll_path:endswith(".dll")) or
                                ((not target:is_plat("windows", "mingw")) and
                                    (dll_path:endswith(".so") or dll_path:match(".+%.so%..+$") or
                                        dll_path:endswith(".dylib")))
                         then
